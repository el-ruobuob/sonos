...
function callBackToSonos(message, lieu) {
  ...
  // On va ajouter un UUID
  var speakUUID = generateUUID();
  // On stoque notre nouveau speak en table
  newSpeak = [speakUUID, lieu, message];
  speaks.push(newSpeak);
...
//   }
  var exec = require('child_process').exec,
    child,
    timeout = (500 * (speaks.length - speaked - 1)) + 1;
//   if (speaking == true) {
//     timeout = 50 * speaks.length; 
//   } else {
//     timeout = 10;
//   }
  setTimeout(function() {
	...
	// On vérifie si on speak déjà
	if (speaking == false) {
	  // Si on est pas en train de parler
	  getTopology(function(){
	    // on récupère le volume actuel
	    getVolume(function(volumeinit) {
	      //original['volume'] = volumeinit;
	      GetTransportStatus (function(statusbefore) {
		//original['status'] = statusbefore;
		GetInfosPosition(function(position) {
  //		original['position'] = position;
  // 		var results = position.trackuri.match(monregex);                 
  // 		if (results != null) {
  // 		  original['activity'] = 'playlist';
  // 		} else {
  // 		  original['activity'] = 'radio';
  // 		}
		  Stop(function () {
		    setVolume(configSonosPerso.volumeAnnonce, function () {
		      AddURIToQueue('http://'+config.http.ip+':'+config.http.port+'/assets/sonos/'+speakUUID+'.wav', function(tracknumbertemp) {
			speaked++;
	    ...
	    AddURIToQueue('http://'+config.http.ip+':'+config.http.port+'/assets/sonos/'+speakUUID+'.wav');
	    speaked++;
	  }	      
      }
    });
  ...
  // Fonction permettant la génération d'UUID
